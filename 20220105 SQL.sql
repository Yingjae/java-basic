...
-- 이 경우 매번 id값을 사용자가 계산해서 insert 해야하기 때문에
...
-- id컬럼에는 AUTO_INCREMENT 가 붙어 있으므로 null 처리해도
...
-- testtbl3의 초기 AUTO_INCREMENT 값을 1500으로 고쳐보세요.
ALTER TABLE testtbl3 AUTO_INCREMENT = 1500;
-- 증가분을 1이 아닌 3으로 세팅하는법
SET @@auto_increment_increment = 3;
-- 동물 3마리를 testtbl3에 추가해보세요.
INSERT INTO testtbl3 VALUES(null, '호랑이', 5);
INSERT INTO testtbl3 VALUES(null, '고양이', 2);
...
INSERT INTO testtbl2 VALUES (null, '게', 4), (null, '꽃게', 1), (null, '톱밥게', 2);
...
-- WHERE을 이용해 age를 필터링 해주세요.
-- 하나의 나이대만 우선 고쳐주세요.
UPDATE testtbl2 SET user_name = '고양이' WHERE age = 2;
SELECT * FROM testtbl2;

-- UPDATE 구문이 전체 컬럼에 적용 될 수 있다는 것을 응용해 특정 컬럼의 값을 일괄적으로
-- 계산식에 따라 새로운 결과값으로 갱신하는 것도 가능합니다.
...
-- DELETE 특정 컬럼만 삭제하는 경우가 없고 삭제할거면 row를 삭제하거나
-- 삭제를 안 해버리거나 이기 때문에
...
-- 따라서 속도를 끌어올리고 싶다면 TRUNCATE를 DELET FROM 대신 쓰는 경우도 있습니다.
...
-- 기존 데이터의 id와 중복되는 값이 있고, 이후 99건은 PK가 중복되지 않는다고 해도ㅓ
-- SQL 시스템상 전체 데이터가 입력되지 않는 문제가 발생합니다.
...
-- MYSQL은 구문을 통째로 묶어서 실행할 경웅 모두 실행하거나 모두 실행하지 않습니다.
...
-- 을 가지고 있는 계정입니다.
-- 그렇지만 모든 사람이 권한을 가지는 일은 그만큼 DB접근이 용이해져 위험을 내포하고 있으며
...
-- 5. Account Limit는 시간당 기능 사용 가능 횟수를 설정 합니다.
-- 6. Administrative Roles는 어떤 구문 실행까지 허용 할지 입니다.

-- CMD 환경에서 생성하는 경우는
-- CREATE USER 계정명@'%' IDENTIFIED BY '비밀번호';
-- GRANT ALL on *.* TO 계정명@'%' WITH GRANT OPTION;
-- 위 두 줄을 이용해서 생성할 수 있습니다.
-- ict2 계정은 CMD 환경에서 여러분들이 만들어주세요. 비밀번호는 ictICT03 입니다.


-- 계정 접속 방법
-- 커맨드 라인 접속시 mysql -u 계정명 -p 로 연결 가능
-- 접속 후 show databases; 를 이용해 DB목록 확인 가능
-- use mysql;
-- SELECT user, host FROM user; 를 할 경우 생성된 계정목록 조회가능
show databases; -- DB목록 조회

-- icttest 계정을 만들어 주세요. 권한은 따로 주지 마세요.

-- 계정 권한주기

-- workbench에서 주기
-- Navigatord 하단 Administration에서 유저 선택 후 Administrative Roles에서 주고 싶은 권한을
-- 체크해주면 됩니다.(DBA 선택시 모든 권한 획득)

-- CMD에서 전체가 아닌 일부 권한 주기
-- icttest 계정을 하나 새로 만들어주세요. 권한 주지마세요.
-- GRANT 권환1, 권한2... ON DB명.테이블명 TO 아이디@'%';
