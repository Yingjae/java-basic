...
-- FROM 구문에서 테이블명만 적는게 아니라, 테이블명 별명 형식으로 적을 경우는
...
-- 가격 남은 자료들을 가격순으로 내림차순 출력해주세요. ORDER BY 가격 내림차순;
SELECT b.order_number, b.user_id, b.prod_name,
	   b.group_name, b.price, b.amount,
	   u.user_name, u.phone_number, u.addr
	  FROM buytbl b INNER JOIN usertbl u
...
-- 왼쪽 테이블 데이터는 오른쪽 기준 컬럼과 일치할때만 출력합니다.
SELECT * FROM usertbl; -- 가입회원은 12명
SELECT * FROM buytbl; -- 구매 이력이 있는 회원은 5명

-- LEFT JOIN인데, usertbl을 LEFT에, buytbl을 RIGHT에 두고 작성해주세요.
-- INNER JOIN을 넣은 자리 대신 LEFT OUTER JOIN으로 고쳐주시기만 하면 작동합니다.
-- 테이블의 컬럼은 전체 출력합니다.
...
-- INNER JOIN을 넣은 자리 대신 RIGHT OUTER JOIN으로 고쳐주시기만 하면 작동합니다.
-- 테이블의 컬럼은 전체 출력합니다.
...
INSERT INTO membership VALUES ('노영웅', 500);
INSERT INTO membership VALUES ('김철수', 37000);
SELECT * FROM membership;    

-- 위 데이터셋(학생 student, 회원포인트 membership)에 대해 LEFT JOIN 해주세요.
