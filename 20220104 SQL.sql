...
-- 가입한 순서(reg_data순으로 오름차순 조회를 해 보겠습니다.)
SELECT * FROM usertbl ORDER BY reg_date DESC;

-- 키 작은 순으로 나열해 주세요.
SELECT * FROM usertbl ORDER BY height ASC;

-- 정렬시 동점인 부분을 처리하기 위해 ORDER BY 절을 두개 이상 동시에 나열 할 수도 있습니다.
...
-- 중복값을 하나로 출력하기 위해 출력컬럼 앞에 distinct 를 붙입니다.
...
-- 만약 limit 숫자1, 숫자 2;와 같이 숫자 2개를 넣는 경우
-- 숫자 1번부터 순자 2에 기입한 갯수만큼 보여줍니다.
SELECT * FROM employees limit 5, 10;


-- GROUP BY는 같은 데이터를 하나의 그룹으로 묶어줍니다.
-- 조건절은 WHERE이 아닌 HAVING으로 처리하게 됩니다.
use ict_practice;
...
-- SELECT user_name,max(height) FROM usertbl;(제대로 된 표시가 안됨)
...
-- 하지만 GROUP BY 를 통해 집계하는 경우 WHERE 절을 받을 수가 없습니다.
-- 대신 조건절을 HAVING 으로 대체해 사용합니다.
-- WHERE 과 사용법은 완전히 동일합니다.
-- 우선 사용자별 총 구매액을(buytbl) 다시 구해보겠습니다.
-- sum(price * amount)를 활용해서 다시 작성해주세요.
SELECT user_id, sum(price * amount) AS '총구매액' 
	FROM buytbl GROUP BY user_id;
    
-- 구매액이 250000원을 넘는 사람만 남겨보겠습니다.
-- WHERE 처럼 사용하시되 HAVING 이라고 적어서 조건식을 붙여보세요.
SELECT user_id, sum(price * amount) AS '총구매액' FROM buytbl 
...
-- 테이블명 뒤에 컬럼정보를 적지 않으면, CREATE TABLE 시 설정한
-- 컬럼 정보가 순서대로 대입됩니다(id, user_name, age 순으로)
INSERT INTO testtbl1 VALUES(1, '홍길동', 25);

-- id와 이름쪽에만 데이터를 넣을 때에는 컬럼명을 생략할 수 없습니다.
INSERT INTO testtbl1 (id, user_name) VALUES (2, '홍길동');
